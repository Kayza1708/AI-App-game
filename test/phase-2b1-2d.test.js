...
test('Capacity is Users and removed allocation has no effect',()=>{const a=marketState(),b=structuredClone(a);b.allocation.inference=1;const ma=marketMetrics(a),mb=marketMetrics(b);assert.equal(ma.users,ma.capacity);close(ma.capacity,mb.capacity);assert.equal(ma.bottleneck,'FULLY UTILIZED')});
...
test('locked Research production is exact, finite, monotonic, concave, and allocation-conserving',()=>{const inputs=[0,100,1_000,10_000,1e6,1e9],modifier=1.37,values=inputs.map(value=>researchPointsPerSecond(value,modifier));inputs.forEach((value,index)=>close(values[index],value===0?0:4*(value/1_000)**.72*modifier));for(let i=1;i<values.length;i++)assert.ok(values[i]>values[i-1]&&Number.isFinite(values[i]));assert.ok(researchPointsPerSecond(2e6)<2*researchPointsPerSecond(1e6));const s=marketState();s.meta.cycles=1;s.meta.techNodes.push('system-research');s.allocation={training:30,inference:40,research:30,data:0,agents:0};assert.ok(researchPerSecond(s)>0);assert.ok(economySnapshot(s).computeConsumed>=economySnapshot(s).computePerSecond)});
