...
const parameters=[
 ...
 parameter('training.durationBaseSeconds',BALANCE.training.durationBaseSeconds,'base + 34√level + transition','TUNABLE_PARAMETER','Training','Lengthens all reference requirements',['static reference rate']),
 parameter('training.durationSqrtCoefficient',BALANCE.training.durationSqrtCoefficient,'45 + coefficient√level + transition','TUNABLE_PARAMETER','Training','Lengthens higher-level targets concavely',['level']),
 parameter('models.levelCoefficient',BALANCE.models.levelCoefficient,'1 + coefficient × L^0.62','TUNABLE_PARAMETER','Models/Market','Raises level contribution to Demand',['model level']),
 parameter('models.levelPower',BALANCE.models.levelPower,'1 + .16 × L^power','TUNABLE_PARAMETER','Models/Market','Raises late-level sensitivity',['model level']),
...
writeFileSync(new URL('../ECONOMY_PARAMETER_MAP.json',import.meta.url),JSON.stringify({schemaVersion:1,generatedAt:new Date().toISOString(),purpose:'Parametric balancing map; AUDIT_ONLY entries are not change authorization.',parameters},null,2)+'\n');
const entries=[
