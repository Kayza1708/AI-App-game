...
export function modifierValue(state,effect,modelId=null){const itemPower=Math.max(.1,1+technologyEffect(state,'itemPower')),artifactPower=Math.max(.1,1+technologyEffect(state,'artifactPower'));return(itemModifiers(state,modelId)[effect]??0)*itemPower+(artifactModifiers(state)[effect]??0)*artifactPower}
