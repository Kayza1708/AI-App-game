...
export function inferCompanyBuild(state){const progress=Object.values(state.model.progress??{}),skill=(...ids)=>progress.reduce((sum,model)=>sum+ids.reduce((points,id)=>points+(model.skills?.[id]??0),0),0),skillPoints=progress.map(model=>Object.values(model.skills??{}).reduce((sum,value)=>sum+value,0));const scores={Compute:modifierValue(state,'hardwareOutput')+modifierValue(state,'training')+state.allocation.training*.002,Research:modifierValue(state,'research')+skill('research','reasoning','knowledge','math')*.025+state.allocation.research*.01,Consumer:modifierValue(state,'adoption')+modifierValue(state,'popularity')+skill('popularity','creativity','vision')*.025+state.allocation.inference*.003+(state.market.priceMultiplier<1?.12:0),Enterprise:modifierValue(state,'enterprise')+modifierValue(state,'revenue')+skill('enterprise','safety','quality')*.025+(state.market.priceMultiplier>1.5?.25:0),Efficiency:modifierValue(state,'inference')+skill('efficiency','latency')*.035,Agent:modifierValue(state,'agents')+skill('autonomy')*.04+state.allocation.agents*.01,'Model Specialist':Math.max(0,...skillPoints)*.04};const ranked=Object.entries(scores).sort((a,b)=>b[1]-a[1]);return ranked[0]?.[1]>.12&&ranked[0][1]>ranked[1][1]*1.2?ranked[0][0]:'Balanced'}
