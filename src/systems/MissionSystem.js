...
function scaledReward(state,reward,period){const credits=Math.ceil(Math.max(BALANCE.missions.creditRewardFloor[period],economySnapshot(state).creditsPerSecond*BALANCE.missions.creditRewardSeconds[period]));return{...reward,credits,gems:Math.max(reward.gems??0,BALANCE.missions.gems[period])}}
function grantReward(state,reward,period){let next=state;if(reward.credits)next={...next,resources:{...next.resources,credits:next.resources.credits+reward.credits},statistics:{...next.statistics,totalCreditsEarned:next.statistics.totalCreditsEarned+reward.credits},run:{...next.run,creditsEarned:next.run.creditsEarned+reward.credits}};if(reward.gems)next=earnGems(next,reward.gems,`${period}-mission`);if(reward.item)next=acquireItem(next,reward.item,'mission');if(reward.cache)next=grantCache(next,reward.cache);return next}
