...
function scaledReward(state,reward,period){if(!reward.credits)return reward;const credits=Math.ceil(Math.max(BALANCE.missions.creditRewardFloor[period],economySnapshot(state).creditsPerSecond*BALANCE.missions.creditRewardSeconds[period]));return{...reward,credits}}
function grantReward(state,reward,source){let next=state;if(reward.credits)next={...next,resources:{...next.resources,credits:next.resources.credits+reward.credits},statistics:{...next.statistics,totalCreditsEarned:next.statistics.totalCreditsEarned+reward.credits},run:{...next.run,creditsEarned:next.run.creditsEarned+reward.credits}};if(reward.gems)next=earnGems(next,reward.gems,source);if(reward.item)next=acquireItem(next,reward.item,source);if(reward.cache)next=grantCache(next,reward.cache);return next}
