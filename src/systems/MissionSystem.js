import { featureUnlocked } from '../config/balance.js';
import { ITEM_CATALOG } from '../data/itemCatalog.js';
import { economySnapshot } from './GameSystem.js';
...
const POOLS = {
  daily: [
    ...
    mission('training','Training','Generate Model XP','modelXp',s=>Math.max(10,s.model.level*16),{cache:'model-cache'}),
    ...
    mission('energy','Energy','Improve Energy production','energyBuilt',()=>1,{cache:'compute-cache'},'energy'),
  ...
  weekly: [
    ...
    mission('weekly-energy','Energy','Construct Energy infrastructure','energyBuilt',()=>3,{gems:3},'energy'),
  ],
  monthly: [
...
export function claimMission(state,id){const mission=missionsWithProgress(state).find(entry=>entry.id===id);if(!mission||mission.claimed||mission.progress<mission.target)return state;let next={...state,missions:{...state.missions,claims:{...state.missions.claims,[id]:Date.now()}},statistics:{...state.statistics,totalMissionsClaimed:state.statistics.totalMissionsClaimed+1}};next=grantReward(next,mission.reward,'mission');return updateDailyStreak(next,mission.period)}
...
function generatePeriod(state,period,id){const available=POOLS[period].filter(entry=>!entry.feature||featureUnlocked(state,entry.feature)),count=period==='daily'?Math.min(4,available.length):period==='weekly'?Math.min(4,available.length):Math.min(2,available.length),random=seeded(seedOf(`${state.profile.localOwnerId}:${id}:${period}`)),selected=[];while(selected.length<count){const candidate=available[Math.floor(random()*available.length)];if(candidate&&!selected.some(entry=>entry.category===candidate.category||entry.key===candidate.key))selected.push(candidate);else if(selected.length>=new Set(available.map(entry=>entry.category)).size){const fallback=available.find(entry=>!selected.includes(entry));if(fallback)selected.push(fallback);else break}}return selected.map((entry,index)=>({id:`${id}-${entry.key}`,period,category:entry.category,text:entry.text,target:Math.ceil(entry.target(state)),metric:entry.metric,baseline:metric(state,entry.metric),reward:entry.reward,bonus:period==='daily'&&index===3}))}
