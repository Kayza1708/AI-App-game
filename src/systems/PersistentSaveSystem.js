...
export class SaveSystem {
  ...
  #mergeWithDefaults(save) {
    ...
    return ensureGameState({
      ...
      patents: { ...defaults.patents, ...readSaveObject(save.patents), discovered: readSaveArray(save.patents?.discovered), equipped: readSaveArray(save.patents?.discovered), slots:Number.MAX_SAFE_INTEGER, history: readSaveArray(save.patents?.history), levels: readSaveObject(save.patents?.levels), intInvested: readSaveObject(save.patents?.intInvested) }, premium: { ...defaults.premium, ...readSaveObject(save.premium), purchases: readSaveArray(save.premium?.purchases), adCooldowns: { ...defaults.premium.adCooldowns, ...readSaveObject(save.premium?.adCooldowns) } },
      ...
      inventory: { ...defaults.inventory, ...readSaveObject(save.inventory), instances: readSaveArray(save.inventory?.instances).filter((item) => item && typeof item.instanceId === 'string' && typeof item.catalogId === 'string'), equipped: readSaveObject(save.inventory?.equipped), components:readSaveNumericMap(save.inventory?.components), componentHistory:readSaveArray(save.inventory?.componentHistory), craftHistory:readSaveArray(save.inventory?.craftHistory), craftedCount:Number.isFinite(save.inventory?.craftedCount)?Math.max(0,Math.floor(save.inventory.craftedCount)):0, collection: { ...defaults.inventory.collection, ...readSaveObject(save.inventory?.collection), items: readSaveArray(save.inventory?.collection?.items), rarities: readSaveArray(save.inventory?.collection?.rarities), sets: readSaveArray(save.inventory?.collection?.sets) }, newItem: null },
...
function normalizeModelPointAccounting(value) { const source=readSaveObject(value.skills),refundedPopularity=skillPointInvestment(source.popularity??0),skills=Object.fromEntries(Object.entries(source).filter(([skill])=>['quality','efficiency'].includes(skill)));const priorSpent=Number.isFinite(value.totalPointsSpent)?value.totalPointsSpent:Object.values(source).reduce((sum,rank)=>sum+skillPointInvestment(rank),0),spent=Math.max(0,priorSpent-refundedPopularity),available=Math.max(0,Math.floor(value.availablePoints??value.upgradePoints??0))+refundedPopularity,trainingCount=Math.max(0,Math.floor(value.trainingCount??value.trainings??0));return{...value,trainings:trainingCount,trainingCount,upgradePoints:available,availablePoints:available,totalPointsSpent:spent,totalPointsEarned:Math.max(Number(value.totalPointsEarned)||0,available+spent),skills}; }
function skillPointInvestment(rank){return Array.from({length:Math.max(0,Math.floor(rank))},(_,index)=>BALANCE.training.pointCosts[index]??Math.ceil((index+1)/2)).reduce((a,b)=>a+b,0)}
...
function normalizeSavedAllocationForFeatures(defaults,value,techNodes,save){const allocation=normalizeSavedAllocation(defaults,value);if(isResearchUnlocked({...save,meta:{...save.meta,techNodes}}))return allocation;return{...allocation,inference:allocation.inference+allocation.research,research:0}}
function normalizeResearchUpgradeLevels(save){const aliases={'research-algorithms':'research-model-1','research-silicon':'research-compute-1','research-behavior':'research-economy-1','research-compression':'research-economy-2','research-economics':'research-economy-3','research-automation':'research-science-1'},explicit=readSaveNumericMap(save.researchUpgradeLevels);for(const id of readSaveArray(save.upgrades))if(aliases[id])explicit[aliases[id]]=Math.max(1,explicit[id]??1);for(const [legacy,current] of Object.entries(aliases))if(explicit[legacy]){explicit[current]=Math.max(explicit[current]??0,explicit[legacy]);delete explicit[legacy]}return explicit}
