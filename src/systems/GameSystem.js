...
export function tickGame(state, deltaMs) {
  ...
  if (trainingActive) {
    ...
    if (trainingProgress >= required) { const startingLevel=trainingSession?.startingLevel??level; trainingProgress = 0; trainingActive = false; completedTraining = true; trainings += 1; xp = 0; level += 1; upgradePoints += 1; lastTrainingResult={...trainingSession,modelId:state.model.activeId,completedAt:Date.now(),completionPlaytimeMs:state.statistics.playTimeMs+deltaMs,actualDuration:(trainingSession?.activeElapsedMs??deltaMs)/1000,effectiveDuration:(trainingSession?.activeElapsedMs??deltaMs)/1000,computeInvested:trainingSession?.computeInvested??computeInvested,requiredCompute:required,startingLevel,resultingLevel:level,upgradePointsGained:1};trainingSession=null; }
  }
