...
export function completeTrainingProgress(progress){const availablePoints=modelAvailablePoints(progress)+1,totalPointsSpent=Math.max(0,progress.totalPointsSpent??0),trainingCount=modelTrainingCount(progress)+1;return{...progress,level:Math.max(1,progress.level??1)+1,xp:0,trainings:trainingCount,trainingCount,upgradePoints:availablePoints,availablePoints,totalPointsSpent,totalPointsEarned:availablePoints+totalPointsSpent}}
