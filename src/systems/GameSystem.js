...
export function purchaseTechnology(state, nodeId) {
  ...
  const featureUnlockTimes=node.unlockFeature?{...state.meta.featureUnlockTimes,[node.unlockFeature]:state.meta.featureUnlockTimes?.[node.unlockFeature]??state.statistics.playTimeMs}:state.meta.featureUnlockTimes;let next={ ...state,statistics:{...state.statistics,totalTechPurchased:(state.statistics.totalTechPurchased??0)+1}, meta: { ...state.meta, intelligence: state.meta.intelligence - node.cost, techNodes: [...state.meta.techNodes, nodeId], featureUnlockTimes } };
