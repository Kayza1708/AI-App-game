...
export function researchUpgradeCost(state,upgrade){const level=state.researchUpgradeLevels?.[upgrade.id]??0;return Math.ceil(upgrade.cost*BALANCE.research.upgradeLevelGrowth**level)}
export function canBuyUpgrade(state, upgrade) { const cost=upgrade.category==='research'?researchUpgradeCost(state,upgrade):upgrade.cost,balance = upgrade.category === 'research' ? state.resources.research : state.resources.credits; const unlocked = upgrade.category === 'hardware' ? state.hardware[upgrade.hardwareId] >= upgrade.unlock : state.model.level >= upgrade.unlock; return (upgrade.category==='research'||!state.upgrades.includes(upgrade.id)) && unlocked && balance >= cost; }
export function buyUpgrade(state, upgradeId) { const upgrade = UPGRADES.find(({ id }) => id === upgradeId); if (!upgrade || !canBuyUpgrade(state, upgrade)) return state;if(upgrade.category==='research'){const cost=researchUpgradeCost(state,upgrade),level=(state.researchUpgradeLevels?.[upgrade.id]??0)+1;return feedback({...state,resources:{...state.resources,research:state.resources.research-cost},researchUpgradeLevels:{...state.researchUpgradeLevels,[upgrade.id]:level},upgrades:[...new Set([...state.upgrades,upgrade.id])]},`${upgrade.name} Level ${level} installed`)}const paid=spendCredits(state,upgrade.cost);return feedback({ ...paid, upgrades: [...paid.upgrades, upgradeId] }, `${upgrade.name} installed`); }
...
export function developmentCyclePreview(state){const gain=cycleIntelligence(state),before=state.meta.totalIntelligence??0,after=before+gain;return{gain,lifetimeBefore:before,lifetimeAfter:after,incomeBonusBefore:before*.1,incomeBonusAfter:after*.1,newSystems:(state.meta.cycles??0)===0?['Research Division']:[]};}
