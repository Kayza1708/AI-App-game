import { ACHIEVEMENTS, createDefaultState, GEM_SHOP_ITEMS, HARDWARE_CATALOG, MODEL_CATALOG, MODEL_SKILLS, OBJECTIVES, PATENTS, TECH_NODES, UPGRADES } from '../data/defaultState.js';
...
import { RESEARCH_PROJECTS, researchEffect, researchMechanic, researchPrerequisitesMet, researchProjectCost, startResearchProject, tickResearchLabs, unlockedResearchLabs } from './ResearchSystem.js';
import { marketSnapshot } from './MarketSystem.js';
import { efficiencyFactor, hardwareUnitCost, qualityRevenueFactor, trainingRequirement } from './ProgressionSystem.js';
...
function strategicBonus(state, effect) {
  ...
  const events = state.world.modifiers.filter((modifier) => modifier.effect === effect && (!modifier.expiresAtEpoch || modifier.expiresAtEpoch > Date.now())).reduce((sum, modifier) => sum + modifier.value, 0);
  ...
  const equipped=PATENTS.filter((patent)=>state.patents.discovered.includes(patent.id));const tagDiversity=new Set(equipped.flatMap(patent=>patent.tags??[])).size;const patentPower=Math.max(.1,1+technologyEffect(state,'patentPower')+(hasTechnologyMechanic(state,'patent-diversity')?Math.max(0,tagDiversity-1)*.03:0));
...
function upgradeBonus(state, effect, hardwareId = null) {
  const purchased=UPGRADES.filter((upgrade)=>upgrade.category!=='research'&&state.upgrades.includes(upgrade.id)&&upgrade.effect===effect&&(!upgrade.hardwareId||upgrade.hardwareId===hardwareId)).reduce((total,upgrade)=>total+upgrade.value,0);
  return purchased+researchEffect(state,effect);
}
...
export function modelComparison(state,modelId){const candidate=MODEL_CATALOG.find(model=>model.id===modelId),current=activeModel(state);if(!candidate)return null;const potential=model=>({revenue:qualityRevenueFactor(model.stats.quality),capacity:efficiencyFactor(model.stats.efficiency),training:model.trainingScale,research:model.stats.research});const before=potential(current),after=potential(candidate),delta=key=>before[key]?after[key]/before[key]-1:after[key]?Infinity:0;return{against:current.name,revenue:delta('revenue'),capacity:delta('capacity'),training:delta('training'),research:delta('research'),trait:candidate.role,bestFor:candidate.archetype??candidate.specialty}}
...
export function effectiveModelStat(state, model, stat) { const base=model.stats[stat]??0; const points=skillUnlocked(state,stat)?modelImprovementLevel(state,model.id,stat):0;const skillPower=Math.max(.1,1+technologyEffect(state,'modelSkillPower')+(stat==='efficiency'?technologyEffect(state,'efficiencyPower')+researchEffect(state,'efficiencyPower'):0)+(stat==='popularity'?technologyEffect(state,'popularityPower'):0)+(stat==='quality'?technologyEffect(state,'qualityPower')+researchEffect(state,'qualityPower'):0));const portfolio=hasTechnologyMechanic(state,'specialist-ai')&&model.id!==state.model.activeId?.5:hasTechnologyMechanic(state,'generalist-ai')?.8:1; return (base + points * BALANCE.training.skillGain*skillPower + modifierValue(state,stat,model.id)) * portfolio * (stat==='quality' ? 1 + strategicBonus(state,'quality') + deployedIdentityBonus(state,'quality') : 1); }
// INT is entitlement and Technology currency, not a second passive power layer.
export function lifetimeIncomeMultiplier() { return 1; }
export function revenuePerUser(state) { const revenueModels=[...new Set([...state.model.deployed,state.model.activeId])],models=revenueModels.map(id=>MODEL_CATALOG.find(item=>item.id===id)).filter(Boolean);const quality=models.reduce((sum,model)=>sum+effectiveModelStat(state,model,'quality'),0)/Math.max(1,models.length);return BALANCE.market.revenueBase*qualityRevenueFactor(quality)*lifetimeIncomeMultiplier(state)*Math.max(.1,1+upgradeBonus(state,'revenue')+milestoneBonus(state,'revenue')+strategicBonus(state,'revenue')+deployedIdentityBonus(state,'revenue')); }
...
function trainingMultiplier(state) { const momentum=hasTechnologyMechanic(state,'training-momentum')?Math.min(.5,(activeProgress(state).trainings??0)*.03):0,gpu=hasTechnologyMechanic(state,'gpu-training')&&HARDWARE_CATALOG.filter(item=>item.tier>=3).some(item=>state.hardware[item.id]>0)?.2:0;return Math.max(.1,1+upgradeBonus(state,'training')+strategicBonus(state,'training')+deployedIdentityBonus(state,'coding')+momentum+gpu); }
export function trainingRatePerSecond(state) { return computePerSecond(state)*BALANCE.training.computeShare*trainingMultiplier(state); }
export function researchPerSecond(state){const compute=researchComputePerSecond(computePerSecond(state),100);return researchPointsPerSecond(compute,Math.max(.1,1+strategicBonus(state,'research')+researchEffect(state,'research')))}
...
function marketContext(state,totalCompute=computePerSecond(state)) {
  const deployed=effectiveMarketModels(state),effectiveEfficiency=deployed.reduce((sum,model)=>sum+effectiveModelStat(state,model,'efficiency'),0)/Math.max(1,deployed.length),quality=deployed.reduce((sum,model)=>sum+effectiveModelStat(state,model,'quality'),0)/Math.max(1,deployed.length);
  return{totalComputePerSecond:totalCompute,modelEfficiency:efficiencyFactor(effectiveEfficiency),inferenceModifiers:BALANCE.market.capacityScale*Math.max(.1,1+upgradeBonus(state,'inference')+strategicBonus(state,'inference')),revenuePerUser:revenuePerUser(state),quality};
}
...
export function createDefaultEconomySnapshot() {
  return {
    ...
    developmentCycle: 0, runCreditsEarned: 0, runComputeProduced: 0, lifetimeQualifyingCompute:0,totalIntEntitlement:0,lifetimeIntClaimed:0,claimableInt:0,lifetimeIntSpent:0,intelligence: 0, gems: 0,
...
export function economySnapshot(input) {
  ...
  // Serving, Training and Research run in parallel.  Allocation is a migrated,
  // inert save field and must never partition the V1 Compute flow.
  const computeConsumed = compute;
  const storedComputeRate = 0;
  ...
  return { ...createDefaultEconomySnapshot(),
    ...
    compute: state.resources.compute, computePerSecond: compute, computeConsumed, computeWasted: 0, storedComputeRate,
    trainingCompute: trainingRate, research: state.resources.research, researchPerSecond: researchPerSecond(state),
    users: market.users, currentUsers:market.users, servedUsers:market.servedUsers, usersPerSecond: 0, organicUsersPerSecond:0, targetUsers: market.users, potentialDemand:market.users, unlockedMarketSize: market.users, demand: market.users, capacity: market.inferenceCapacity, inferenceComputePerSecond:market.inferenceComputePerSecond,
    utilization: market.utilization, revenuePerUser: revenuePerUser(state), marketFactors:market.factors,
    ...
    developmentCycle: state.meta.cycles, runCreditsEarned: state.run.creditsEarned, runComputeProduced: state.run.computeProduced, lifetimeQualifyingCompute:lifetimeQualifyingCompute(state),totalIntEntitlement:intEntitlement(state,prestigeModifier(state)),lifetimeIntClaimed:state.meta.lifetimeIntClaimed??0,claimableInt:claimableInt(state,prestigeModifier(state)),lifetimeIntSpent:state.meta.lifetimeIntSpent??0,intelligence: state.meta.intelligence, gems: state.resources.gems,
...
export const TUTORIAL_STEPS = Object.freeze([
  ...
  {id:'compute',title:'COMPUTE IS FLOWING',copy:'Compute/s sets User capacity. Every available slot is automatically filled.',feature:'core',eligible:state=>viewUnlocked(state,'dashboard'),condition:state=>state.statistics.totalComputeProduced>=.25},
  ...
  {id:'model-point',title:'IMPROVE THE MODEL',copy:'Spend each Model Point on Quality for Revenue/User or Efficiency for more Users.',feature:'modelSkills',eligible:state=>viewUnlocked(state,'model'),condition:state=>totalModelPointsSpent(state)>0},
  {id:'users',title:'USERS CREATE CREDITS',copy:'Users always fill Capacity and automatically generate Credits every second.',feature:'core',eligible:state=>viewUnlocked(state,'dashboard'),condition:state=>state.resources.users>=1},
...
export function tickGame(state, deltaMs, options={}) {
  ...
  if (options.integrate!==false&&deltaMs > BALANCE.training.maximumSimulationStepMs) {
  ...
  const rawTrainingGain = produced * BALANCE.training.computeShare;
  ...
  if (trainingActive) {
    ...
    trainingProgress += trainingGain + storedTrainingUsed * effectiveTrainingMultiplier * (1+researchEffect(state,'storedComputeEfficiency'));
  ...
  if (nextPatent && patentProgress >= patentResearchRequired(discoveredPatents.length)) { patentProgress -= patentResearchRequired(discoveredPatents.length); discoveredPatents = [...discoveredPatents, nextPatent.id]; patentHistory = [...patentHistory, { id: nextPatent.id, discoveredAt: Date.now(), cycle: state.meta.cycles }]; equippedPatents = [...discoveredPatents]; patentDiscovery = nextPatent; }
  const eventCountdown = state.world.nextEventMs;
  const event = state.world.activeEvent;
  let next = {
    ...state,
    resources: { ...state.resources, credits: state.resources.credits + creditGain, compute: Math.max(0, state.resources.compute - storedTrainingUsed), users, research: patentSpend.points },
    ...
    statistics: { ...state.statistics, totalCreditsEarned: state.statistics.totalCreditsEarned + creditGain, creditSources:addCreditSource(state.statistics.creditSources,'user-revenue',creditGain), totalComputeProduced: state.statistics.totalComputeProduced + produced, totalComputeConsumed: state.statistics.totalComputeConsumed + produced + storedTrainingUsed, totalComputeWasted: state.statistics.totalComputeWasted,totalUsersServed:(state.statistics.totalUsersServed??0)+metrics.servedUsers*seconds,totalTrainings:(state.statistics.totalTrainings??0)+(completedTraining?1:0),totalModelLevels:(state.statistics.totalModelLevels??0)+(completedTraining?1:0),totalPatentsDiscovered:(state.statistics.totalPatentsDiscovered??0)+(patentDiscovery?1:0), playTimeMs: state.statistics.playTimeMs + deltaMs },
  ...
  const completedMission=missionsWithProgress(next).find(mission=>mission.completed&&!mission.claimed&&(missionsBefore.find(entry=>entry.id===mission.id)?.progress??0)<mission.target);if(completedMission)next={...next,ui:{...next.ui,toast:{message:`Mission complete · ${completedMission.title}`,id:Date.now()}}};
...
export function optimizeGain(state) {
  ...
  return Math.max(0.5,(Math.max(tap.base+flat+datacenterAssist,rate*activeShare))*scaling*patentTap*Math.max(0.1,1+strategicBonus(state,'click'))*trainingInjection*(1+researchEffect(state,'click')));
}
...
export function trainModel(state) {
  ...
  const trainingSession={modelId:state.model.activeId,startedAt:Date.now(),startPlaytimeMs:state.statistics.playTimeMs,startingLevel:activeProgress(state).level,baseRequired:required,expectedDuration:required/Math.max(.0001,rate),activeElapsedMs:0,computeInvested:0,modifiers:{trainingMultiplier:multiplier}};
...
export function togglePatentEquipped(state, patentId) { return state.patents.discovered.includes(patentId)?{...state,patents:{...state.patents,equipped:[...state.patents.discovered]}}:state; }
...
export const PATENT_SLOT_PRICES = Object.freeze({});
export function buyPatentSlot(state) { return state; }
...
export function buyMarketing(state) { if (!featureUnlocked(state,'marketing')) return state; const cost=marketingCost(state);if(state.resources.credits<cost)return state;const paid=spendCredits(state,cost),preview=marketingPurchasePreview(state);return feedback({ ...paid, market: { ...paid.market, marketing: paid.market.marketing + 1 },statistics:{...paid.statistics,totalMarketingPurchased:(paid.statistics.totalMarketingPurchased??0)+1} }, `Marketing Level ${preview.levelAfter} · +${Math.round(preview.demandAfter-preview.demandBefore)} Demand`); }

export function researchUpgradeCost(state,upgrade){const project=RESEARCH_PROJECTS.find(item=>item.id===(upgrade?.id??upgrade));return project?researchProjectCost(state,project):Infinity}
export function canBuyUpgrade(state, upgrade) { const project=RESEARCH_PROJECTS.find(item=>item.id===upgrade?.id);if(project)return researchPrerequisitesMet(state,project)&&(state.researchUpgradeLevels?.[project.id]??0)<project.maxLevel&&state.resources.research>=researchProjectCost(state,project)&&state.researchLabs?.labs?.some(lab=>lab.id<=unlockedResearchLabs(state)&&!lab.projectId);if(!upgrade)return false;const unlocked=upgrade.category==='hardware'?state.hardware[upgrade.hardwareId]>=upgrade.unlock:state.model.level>=upgrade.unlock;return !state.upgrades.includes(upgrade.id)&&unlocked&&state.resources.credits>=upgrade.cost}
export function buyUpgrade(state, upgradeId) { const project=RESEARCH_PROJECTS.find(item=>item.id===upgradeId);if(project){if(!canBuyUpgrade(state,project))return state;const lab=state.researchLabs.labs.find(entry=>entry.id<=unlockedResearchLabs(state)&&!entry.projectId);return lab?feedback(startResearchProject(state,upgradeId,lab.id),`${project.name} assigned to Lab ${lab.id}`):state}const upgrade=UPGRADES.find(({id})=>id===upgradeId);if(!upgrade||!canBuyUpgrade(state,upgrade))return state;const paid=spendCredits(state,upgrade.cost);return feedback({...paid,upgrades:[...paid.upgrades,upgradeId]},`${upgrade.name} installed`)}
...
export function objectiveProgress(state, objective) {
  ...
  const value = metric.startsWith('hardware:') ? state.hardware[metric.slice(9)] ?? 0 : {
    ...
    trainings:state.statistics.totalTrainings??progress.reduce((sum,item)=>sum+(item.trainingCount??item.trainings??0),0),pointsSpent:state.statistics.totalModelPointsSpent??progress.reduce((sum,item)=>sum+(item.totalPointsSpent??0),0),totalCompute:state.statistics.totalComputeProduced,computeRate:computePerSecond(state),creditsEarned:state.statistics.totalCreditsEarned,creditsRate:marketMetrics(state).revenue,totalHardware:Object.values(state.hardware).reduce((sum,owned)=>sum+owned,0),
...
function achievementMetric(state, metric) {
  ...
  if(metric.startsWith('hardware:'))return state.hardware[metric.slice(9)]??0;
  const model=activeModel(state),quality=effectiveModelStat(state,model,'quality'),efficiency=effectiveModelStat(state,model,'efficiency');
  return { totalCreditsEarned: state.statistics.totalCreditsEarned, totalComputeProduced: state.statistics.totalComputeProduced, totalClicks: state.statistics.totalClicks, users: state.resources.users, quality, efficiency, balancedModel:Math.min(quality,efficiency), hardware, level: state.model.level, research: state.resources.research, cycles: state.meta.cycles, patents:state.patents.discovered.length }[metric]??0;
}
...
function awardAchievements(state) {
  ...
  const gems = earned.reduce((sum,achievement)=>sum+(achievement.gemReward??0),0);
...
function applyAutomation(state) {
  let next = state;
  if ((hasTechnologyMechanic(state,'auto-buy')||researchMechanic(state,'research-auto-buy')) && state.session.elapsedMs - state.automation.lastHardwarePurchaseMs >= 1_000) {
  ...
  if ((hasTechnologyMechanic(state,'auto-training')||researchMechanic(state,'research-auto-train')) && !next.model.trainingActive && computePerSecond(next) > 0) next = trainModel(next);
  if(researchMechanic(next,'research-auto-model')){const progress=activeProgress(next),skill=(progress.skills?.quality??0)<=(progress.skills?.efficiency??0)?'quality':'efficiency';next=upgradeModelSkill(next,next.model.activeId,skill)}
  if(researchMechanic(next,'research-auto-research')){const lab=next.researchLabs.labs.find(entry=>entry.id<=unlockedResearchLabs(next)&&!entry.projectId),project=RESEARCH_PROJECTS.filter(item=>researchPrerequisitesMet(next,item)&&(next.researchUpgradeLevels?.[item.id]??0)<item.maxLevel&&researchProjectCost(next,item)<=next.resources.research).sort((a,b)=>researchProjectCost(next,a)-researchProjectCost(next,b))[0];if(lab&&project)next=startResearchProject(next,project.id,lab.id)}
  return next;
}

function prestigeModifier(state){return Math.min(BALANCE.intelligence.prestigeModifierCap,Math.max(0,1+strategicBonus(state,'intelligenceGain')))}
export function developmentCycleRequirements(state){const hardwareTier=HARDWARE_CATALOG.reduce((highest,item)=>state.hardware[item.id]>0?Math.max(highest,item.tier):highest,0),modelLevel=Math.max(state.model.level??1,...Object.values(state.model.progress??{}).map(progress=>progress.level??1)),objectives=Object.values(state.objectives??{}).filter(Boolean).length,claimable=claimableInt(state,prestigeModifier(state));const values={modelLevel,hardwareTier,runCompute:state.run.computeProduced,runCredits:state.run.creditsEarned,objectives,claimableInt:claimable},targets={modelLevel:BALANCE.intelligence.minimumModelLevel,hardwareTier:BALANCE.intelligence.minimumHardwareTier,runCompute:BALANCE.intelligence.computeScale,objectives:BALANCE.intelligence.minimumObjectives,claimableInt:1},met=Object.fromEntries(Object.keys(targets).map(key=>[key,values[key]>=targets[key]]));return{values,targets,met,informational:{runCredits:state.run.creditsEarned},ready:Object.values(met).every(Boolean)}}
export function cycleIntelligence(state) { return developmentCycleRequirements(state).ready?claimableInt(state,prestigeModifier(state)):0; }
export function developmentCyclePreview(state){const gain=cycleIntelligence(state),before=state.meta.totalIntelligence??0,after=before+gain,entitlement=intEntitlement(state,prestigeModifier(state));return{gain,lifetimeBefore:before,lifetimeAfter:after,currentIntelligence:state.meta.intelligence,qualifyingCompute:lifetimeQualifyingCompute(state),totalEntitlement:entitlement,alreadyClaimed:state.meta.lifetimeIntClaimed??0,claimable:Math.max(0,entitlement-(state.meta.lifetimeIntClaimed??0)),newSystems:(state.meta.cycles??0)===0?['Permanent Technology','Research Technology Path']:[]};}
...
export function startDevelopmentCycle(state) {
  ...
  const gained = cycleIntelligence(state); const fresh = createDefaultState(),entitlement=intEntitlement(state,prestigeModifier(state));
  const unlocked=state.meta.unlockedModels??state.model.owned;const progress=Object.fromEntries(unlocked.map(id=>[id,{level:1,xp:0,upgradePoints:0,availablePoints:0,trainings:0,trainingCount:0,totalPointsEarned:0,totalPointsSpent:0,skills:{}}]));const tinyProgress=progress.tinyChat??fresh.model.progress.tinyChat,total=state.meta.totalIntelligence+gained;const featureUnlockTimes={...(state.meta.featureUnlockTimes??{})};for(const feature of FEATURE_UNLOCKS)if(feature.int<=total&&featureUnlockTimes[feature.id]===undefined)featureUnlockTimes[feature.id]=state.statistics.playTimeMs;if((state.meta.cycles??0)===0)featureUnlockTimes.allocation??=state.statistics.playTimeMs;return feedback({ ...fresh, profile:state.profile, resources: { ...fresh.resources, gems: state.resources.gems }, objectives: fresh.objectives, settings: state.settings, session: { ...fresh.session, elapsedMs: state.session.elapsedMs,taps:state.session.taps,manualComputeGenerated:state.session.manualComputeGenerated }, run: { ...fresh.run, number: (state.run.number??state.meta.cycles+1)+1, startedAtSessionMs: state.session.elapsedMs }, statistics: state.statistics, meta: { ...state.meta, unlockedModels:unlocked, intelligence: state.meta.intelligence + gained, totalIntelligence: total, lifetimeIntClaimed:entitlement,totalIntEntitlement:entitlement, cycles: state.meta.cycles + 1, featureUnlockTimes, cycleHistory:[...(state.meta.cycleHistory??[]),{at:state.statistics.playTimeMs,duration:state.session.elapsedMs-(state.run.startedAtSessionMs??0),compute:state.run.computeProduced,lifetimeQualifyingCompute:lifetimeQualifyingCompute(state),totalEntitlement:entitlement,intelligence:gained}] }, model:{...fresh.model,level:tinyProgress.level,xp:tinyProgress.xp,upgradePoints:tinyProgress.upgradePoints,owned:unlocked,deployed:['tinyChat'],progress}, patents: state.patents, premium: state.premium, retention: state.retention, inventory:state.inventory, consumables:state.consumables, rewardCaches:state.rewardCaches, missions:state.missions, gemEconomy:state.gemEconomy, rewardedBoosts:state.rewardedBoosts, artifacts:state.artifacts, marketplace:state.marketplace, futureMeta:state.futureMeta, balanceRun:state.balanceRun, tutorial: state.tutorial,intro:state.intro }, `Development Cycle ${state.meta.cycles+1} complete · +${gained} INT · operational resources reset; permanent knowledge remains`);
}
...
export function purchaseTechnology(state, nodeId) {
  ...
  const featureUnlockTimes=node.unlockFeature?{...state.meta.featureUnlockTimes,[node.unlockFeature]:state.meta.featureUnlockTimes?.[node.unlockFeature]??state.statistics.playTimeMs}:state.meta.featureUnlockTimes;let next={ ...state,statistics:{...state.statistics,totalTechPurchased:(state.statistics.totalTechPurchased??0)+1}, meta: { ...state.meta, intelligence: state.meta.intelligence - node.cost, lifetimeIntSpent:(state.meta.lifetimeIntSpent??0)+node.cost, techNodes: [...state.meta.techNodes, nodeId], featureUnlockTimes } };
