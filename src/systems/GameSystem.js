...
export function marketMetrics(input) {
  const state = ensureGameState(input);
  const effectiveDeployments=[...new Set([...state.model.deployed,state.model.activeId])];
  const deployed = MODEL_CATALOG.filter((model) => effectiveDeployments.includes(model.id));
...
export function optimizeGain(state) {
  ...
  return Math.max(0.5, Math.max(1.5 + state.model.level * 0.15, rate * activeShare) * Math.max(0.1, 1 + strategicBonus(state, 'click'))*trainingInjection);
}
...
function spendGems(state,cost,source){if(cost<=0||state.resources.gems<cost)return state;return{...state,resources:{...state.resources,gems:state.resources.gems-cost},gemEconomy:{...state.gemEconomy,spent:state.gemEconomy.spent+cost,history:[...state.gemEconomy.history,{type:'spent',amount:cost,source,at:Date.now()}].slice(-100)}}}
...
function awardAchievements(state) {
  ...
  return feedback({ ...state, resources: { ...state.resources, gems: state.resources.gems + gems }, gemEconomy:gems?{...state.gemEconomy,earned:state.gemEconomy.earned+gems,history:[...state.gemEconomy.history,{type:'earned',amount:gems,source:'achievement',at:Date.now()}].slice(-100)}:state.gemEconomy, meta: { ...state.meta, achievements: { ...state.meta.achievements, ...Object.fromEntries(earned.map((achievement) => [achievement.id, Date.now()])) } } }, `${earned[0].name} achieved · permanent company bonus${gems ? ` · +${gems} Gem` : ''}`);
}
...
export function cycleIntelligence(state) { const tier=HARDWARE_CATALOG.reduce((highest,item)=>state.hardware[item.id]>0?Math.max(highest,item.tier):highest,0),highestModel=Math.max(state.model.level??1,...Object.values(state.model.progress??{}).map(progress=>progress.level??1));if(tier<BALANCE.intelligence.minimumHardwareTier||highestModel<BALANCE.intelligence.minimumModelLevel||state.run.computeProduced<BALANCE.intelligence.computeScale||state.run.creditsEarned<BALANCE.intelligence.creditScale)return 0;const computeOrders=Math.log10(Math.max(1,state.run.computeProduced/BALANCE.intelligence.computeScale)),creditOrders=Math.log10(Math.max(1,state.run.creditsEarned/BALANCE.intelligence.creditScale)),milestones=Math.max(0,tier-BALANCE.intelligence.minimumHardwareTier)*.45+Math.max(0,highestModel-BALANCE.intelligence.minimumModelLevel)*.12,technology=Math.max(.25,1+strategicBonus(state,'intelligenceGain')*.35);return Math.max(1,Math.floor((1+computeOrders*.8+creditOrders*.45+milestones)*technology*BALANCE.intelligence.breakthroughMultiplier**(state.meta.breakthroughs??0))); }
...
export function worldEventChoiceCost(state,choice){if(!choice?.cost)return 0;const severity=choice.severity??'moderate',incomeSeconds=BALANCE.events.incomeSeconds[severity]??BALANCE.events.incomeSeconds.moderate,share=BALANCE.events.creditShare[severity]??BALANCE.events.creditShare.moderate;return Math.ceil(Math.max(choice.cost,economySnapshot(state).creditsPerSecond*incomeSeconds,state.resources.credits*share));}
