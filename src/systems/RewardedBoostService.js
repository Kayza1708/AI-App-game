...
export class RewardedBoostService {
  ...
  activate(state,id,now=Date.now()){
    ...
    let next={...state,rewardedBoosts:{...state.rewardedBoosts,periodId:period,started:state.rewardedBoosts.started+1,completed:state.rewardedBoosts.completed+1,claims:{...existing,[id]:{count:claim.count+1,lastAt:now}}}};
