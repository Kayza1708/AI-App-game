...
export function createGameplaySnapshot(input, seconds, context = {}) {
  ...
  return {
    ...
    totalLifetimePlaytime: state.statistics.playTimeMs / 1000,
    ...
    trainingProgress: state.model.trainingProgress, trainingRequirement:trainingRequiredForState(state), trainingRate, trainingActive: state.model.trainingActive,
    ...
    modelEfficiency:effectiveModelStat(state,MODEL_CATALOG.find(model=>model.id===state.model.activeId)??MODEL_CATALOG[0],'efficiency'),
    modelPopularity:effectiveModelStat(state,MODEL_CATALOG.find(model=>model.id===state.model.activeId)??MODEL_CATALOG[0],'popularity'),
    hardwareOwnership:{...state.hardware},
    currentModelTier: Math.max(0, MODEL_CATALOG.findIndex(({ id }) => id === state.model.activeId)),
    intelligenceReward: cycleIntelligence(state), developmentCycleEligible: canDevelop(state),developmentCycleEligibleAt:canDevelop(state)?context.timestamp??Date.now():null,
    lifetimeQualifyingCompute:economy.lifetimeQualifyingCompute,totalIntEntitlement:economy.totalIntEntitlement,lifetimeIntClaimed:economy.lifetimeIntClaimed,claimableInt:economy.claimableInt,currentIntelligence:economy.intelligence,lifetimeIntSpent:economy.lifetimeIntSpent,
