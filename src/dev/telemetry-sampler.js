...
function currentHardwareMultiplier(state,economy){const raw=HARDWARE_CATALOG.reduce((sum,item)=>sum+rawHardwareContribution(state,item),0);return raw?economy.computePerSecond/raw:economy.energyEfficiency}
function hardwareTelemetry(state,economy){const multiplier=currentHardwareMultiplier(state,economy);return Object.fromEntries(HARDWARE_CATALOG.map(item=>{const raw=rawHardwareContribution(state,item);return[item.id,{tier:item.tier,owned:state.hardware[item.id],computeContribution:raw*multiplier,energyUsage:item.energy*state.hardware[item.id],effectivePerUnit:state.hardware[item.id]?raw*multiplier/state.hardware[item.id]:item.computePerSecond*multiplier}]}))}
