...
export const TECHNOLOGY_BRANCHES = Object.freeze({
  compute:B('Compute Empire','compute','#55e8da',1,[
    ...
    N('Training Scheduler','major','training',.12,'Accelerates automatic model Training throughput.',{mechanic:'load-balancing',synergy:'Hardware Compute supports automatic Training.'}),
  ...
  hardware:B('Hardware Engineering','hardware','#54a7ff',3,[
    ...
    N('Vertical Integration','keystone','hardwareCost',.30,'Own the supply chain and reduce Hardware prices.',{tradeoff:'revenue',penalty:.20,mechanic:'vertical-integration'}),
  ...
  consumer:B('Consumer AI','consumer','#ff72bc',1,[
    N('Revenue Analytics','minor','revenue',.20,'Improves Credits per User using live product analytics.'),
    N('Revenue Optimization','major','revenue',.18,'Improves automatic Credits generated by Users.',{mechanic:'viral-loops'}),
    N('Efficient Serving','major','inference',.25,'Large User populations improve inference efficiency.'),
    N('Quality Monetization','major','revenue',.22,'Model Quality contributes more strongly to Revenue/User.',{mechanic:'quality-revenue'}),
    N('Efficient Serving','major','revenue',.30,'Improves Efficiency for serving more Users.',{mechanic:'efficiency-mastery'}),
    N('Premium AI','keystone','qualityPower',1,'Doubles Quality effects with a Revenue tradeoff.',{tradeoff:'revenue',penalty:.30,mechanic:'viral-ai'}),
    N('Scale Economics','keystone','revenue',2,'Triples User revenue while sacrificing Enterprise value.',{tradeoff:'enterprise',penalty:.50,mechanic:'mass-market'}),
  ]),
  enterprise:B('Enterprise AI','enterprise','#ffb15a',12,[
    ...
    N('Enterprise First','keystone','revenue',1.5,'Multiply Revenue/User by 2.5 while reducing general User revenue.',{tradeoff:'revenue',penalty:.50,mechanic:'enterprise-first'}),
  ...
  research:B('Research Lab','research','#74d490',3,[
    N('Research Division','system','research',.15,'Unlocks Research production from total Hardware Compute.',{unlockFeature:'research',unlock:'Automatic Research'}),
  ...
  patent:B('Patent Engineering','patent','#65dfbe',35,[
    ...
    N('Patent Amplification','major','patentPower',1,'Strengthens every permanently active Patent.',{mechanic:'patent-slot'}),
    ...
    N('Narrow Portfolio','keystone','patentPower',1,'Permanently active Patent effects are doubled.',{tradeoff:'patentPower',penalty:6,mechanic:'narrow-patents'}),
    N('Diversified Portfolio','major','patentPower',3,'Diversifies permanent Patent effects at reduced strength.',{tradeoff:'patentPower',penalty:.15,mechanic:'diverse-patents'}),
  ]),
  efficiency:B('Efficiency AI','efficiency','#62c7ff',2,[
    ...
    N('Dense Serving','major','inference',.80,'Massively improves User Capacity with a revenue tradeoff.',{tradeoff:'revenue',penalty:.15,mechanic:'maximum-utilization'}),
  ]),
  automation:B('Autonomous Company','automation','#9da9ff',100,[
    N('Smart Automation','system','automation',.10,'Automatically purchases and trains around current bottlenecks.',{mechanic:'smart-automation'}),
    ...
    N('Automated Revenue','major','revenue',.20,'Automatically optimizes Credits per User.',{mechanic:'auto-revenue'}),
  ...
  agent:B('Agent Economy','agents','#df89ff',250,[
    ...
    N('Agent Tasks','system','agents',.15,'Unlocks automated Agent workloads.',{unlockFeature:'agents',unlock:'Automated Agent Tasks'}),
    ...
    N('Agent Swarm','keystone','agents',1,'Double Agent effects while weakening direct revenue.',{tradeoff:'revenue',penalty:.40,mechanic:'agent-swarm'}),
  ...
  data:B('Data & Quality','data','#ff7593',500,[
    ...
    N('Quality Flywheel','major','revenue',.25,'Research and Quality reinforce one another.',{mechanic:'data-flywheel'}),
  ...
  market:B('Revenue & Users','market','#ffcb66',1,[
    N('Revenue Analytics','minor','revenue',.85,'Improves Credits generated by each User.'),
    N('Quality Reputation','major','revenue',.20,'Adds durable Revenue/User.'),
    N('Serving Loops','major','revenue',.24,'Quality and Efficiency reinforce the core loop.',{mechanic:'quality-efficiency'}),
    N('Quality Pricing','major','priceElasticity',.25,'Improves Quality-derived Revenue/User.'),
    N('Global Expansion','major','marketSize',.40,'Opens larger addressable markets.'),
    N('High-Throughput Serving','keystone','revenue',2,'Triple inference effectiveness while reducing Revenue/User.',{tradeoff:'revenue',penalty:.25,mechanic:'growth-at-all-costs'}),
    N('Compute Economy','era','revenue',.75,'Multiplies value generated by the core Compute economy.',{cost:1e27,unlock:'Global market era'}),
  ]),
