...
export function validateGameState(state, economy = null) {
  ...
  for (const [modelId, progress] of Object.entries(state?.model?.progress ?? {})) {const available=progress.availablePoints??progress.upgradePoints;if(!Number.isInteger(available)||available<0||!Number.isInteger(progress.totalPointsEarned)||!Number.isInteger(progress.totalPointsSpent)||progress.totalPointsSpent<0||progress.totalPointsSpent>progress.totalPointsEarned||progress.totalPointsEarned!==available+progress.totalPointsSpent)failures.push(`${modelId} Model Point accounting is invalid`);if(!isRecord(progress.skills)||Object.values(progress.skills).some(level=>!Number.isInteger(level)||level<0))failures.push(`${modelId} Model skill level is invalid`)}
